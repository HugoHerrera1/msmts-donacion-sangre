...
public interface DonacionSangreRepositorio extends JpaRepository<MtstVolanteDonacionSangre,Integer> {
  //  @Cacheable("volanteDonacionByFechas")
    @Query(value = "select * from MTST_VOLANTE_DONACION_SANGRE mds where mds.FEC_EFEC between ?1 and ?2 " +
            " and mds.DES_NSS_PACIENTE = ?3 and mds.DES_AGREGADO_MEDICO = ?4 and mds.IND_ACTIVO =1", nativeQuery = true)
    List<MtstVolanteDonacionSangre> findVolantesDSBetwenFechas(String fechaInicial, String fechaFinal, String nssPaciente, String agregadoMedico);
  //  @Cacheable("volanteDonacionById")
  ...
  //  @Cacheable("volanteDonacionParameters")
